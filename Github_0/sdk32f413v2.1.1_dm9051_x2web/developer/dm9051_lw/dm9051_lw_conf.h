...
SG_FUNCTION(enable_t, test_plan_include);
SG_FUNCTION(enable_t, test_plan_log);

IS_FUNCTION(uint8_t, iomode);
IS_FUNCTION(uint8_t, promismode);
IS_FUNCTION(csmode_t, csmode);
IS_FUNCTION(ncrmode_t, ncrmode);
IS_FUNCTION(enable_t, rxtypemode);
IS_FUNCTION(enable_t, flowcontrolmode); //enable_t
IS_FUNCTION(enable_t, rxmode_checksum_offload); //enable_t
IS_FUNCTION(enable_t, onlybytemode);

IS_FUNCTION(uint8_t, hdir_x2ms);
IS_FUNCTION(enable_t, hdlr_confrecv);
IS_FUNCTION(enable_t, tx_endbit);
IS_FUNCTION(enable_t, generic_core_rst);
...
#define OPT_CONFIRM(name)			OPTS_DATA(enable_t, name)() //appcall (define isonlybytemode(), or OPT_CONFIRM(onlybytemode))
