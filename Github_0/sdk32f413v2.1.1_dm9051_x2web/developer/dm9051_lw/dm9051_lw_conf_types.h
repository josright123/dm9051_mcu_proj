...
typedef struct {
	...
	enable_t test_plan_log; //log when test_plan func, WE also use in x2web project.
	...
	IS_OPTS_DATA(enable_t, rxtypemode)
	IS_OPTS_DATA(enable_t, rxmode_checksum_offload)
	IS_OPTS_DATA(enable_t, flowcontrolmode)
	IS_OPTS_DATA(enable_t, onlybytemode)

	IS_OPTS_DATA(uint8_t, hdir_x2ms)
	IS_OPTS_DATA(enable_t, hdlr_confrecv)
	IS_OPTS_DATA(enable_t, tx_endbit)

	IS_OPTS_DATA(enable_t, generic_core_rst)
} optsex_t;
