...
#define	function_monitor_rx_all(hdspc, headstr, buffer, len) \
	do { \
	if (headstr) \
	  sprint_hex_dump0(hdspc, 0, headstr, len, 32, buffer, 0, /*len*/ 38, DM_TRUE); /*, DM_TRUE, DGROUP_NONE */ \
	else \
	  sprint_hex_dump0(hdspc, 0, "dm9 monitor <<rx  ", len, 32, buffer, 0, /*len*/ 38, DM_TRUE); /*, DM_TRUE, DGROUP_NONE */ \
  /* dm_check_rx(buffer, len); */ \
	} while(0)
